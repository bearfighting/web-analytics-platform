...
## PR5 Conversions and Funnels implementation

Deployment-owned, versioned rules are loaded from `ANALYTICS_DEFINITIONS_FILE` (default `config/analytics-definitions.json`) by the Processor and Analytics API at startup. Conversion property predicates use type-sensitive equality for top-level JSON scalars. Conversion facts count matching deduplicated events and report session conversion rates. Funnel steps are strictly ordered within an existing anonymous Session; the first step's UTC date is the cohort date, and sessions are split at UTC midnight. Rules changed by deployment are applied to history through `processor --rebuild-conversion-funnels --site-id <site_id>`. Reports expose definition IDs/version, daily event/session counts, conversion rates and independent freshness watermarks; properties and Visitor IDs stay out of API responses and Dashboard.
