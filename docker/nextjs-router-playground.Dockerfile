...
CMD ["sh", "-c", "pnpm install --frozen-lockfile && pnpm --filter @web-analytics/observer-next build && exec pnpm --filter @web-analytics/nextjs-router-playground dev --hostname 0.0.0.0"]
